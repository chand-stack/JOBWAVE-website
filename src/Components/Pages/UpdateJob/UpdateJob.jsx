...
const UpdateJob = () => {
  ...
  if(isLoading){
    ...
    const updateJobHandler = (e) => {
        ...
        axios.patch(`/view-job/update/${jobDetail?.data?._id}`, job)
          .then((res) => {
            if (res.data) {
              Swal.fire({
                icon: 'success',
...
<form onSubmit={updateJobHandler} className=' space-y-3 px-10 md:px-5 lg:px-28 border-2 rounded-lg py-10 m-5 lg:m-10 border-[#7367F0]'>
    ...
    <div>
        ...
        <input type="text" defaultValue={jobDetail?.data?.title} placeholder="Job title here" name='title' className="input input-bordered input-primary w-full" />
    </div>
    <div>
        <h1 className='text-white text-xl font-semibold'>Picture URL of the Job Banner</h1>
        <input type="url" name='photo' defaultValue={jobDetail?.data?.photo} placeholder="Job Banne URL" className="input input-bordered input-primary w-full" />
    </div>
    <div>
    ...
    <h1 className='text-white text-xl font-semibold'>Job Category</h1>
        <select defaultValue={jobDetail?.data?.category} className="select select-primary w-full" onChange={categoryHandler}>
        <option disabled defaultValue={jobDetail?.data?.category}  selected>{jobDetail?.data?.category}</option>
  ...
  <option value={'hybrid'}>Hybrid</option>
    ...
    <div>
        ...
        <input type="text" name='salary' defaultValue={jobDetail?.data?.salary} placeholder="E.g. 30,000-60,000" className="input input-bordered input-primary w-full" />
    </div>
    ...
    <div>
        ...
        <textarea name='description' defaultValue={jobDetail?.data?.description} className="textarea textarea-primary w-full" placeholder="Description"></textarea>
    </div>
    <div>
