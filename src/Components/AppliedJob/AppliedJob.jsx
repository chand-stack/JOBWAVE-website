...
const AppliedJob = () => {
    ...
    const {data:appliedjob, isLoading} = useQuery({
        ...
        queryFn: async () => {
           const res = await axios.get(`/apply-job?email=${email}&category=${category}`)
...
{
    appliedJobs.length==0 ? <div className='flex justify-center'>
...

{
    appliedJobs?.map(job => <div key={job._id} className="flex items-center justify-between rounded-lg lg:justify-around border p-3 md:p-10">
        <div>
            <img className="max-h-56 mx-auto rounded-full" src={job?.photo} alt="" />
        </div>
        <div className="space-y-5">
        <h1 className="text-white text-lg font-semibold">
            <span className="text-2xl text-[#A582F7]">Position:</span> {job?.title}
        </h1>
        <h1 className="text-white text-lg font-semibold ">
            <span className="text-2xl text-[#A582F7]">Category:</span> <span className="border py-2 px-4 text-white">{job?.category}</span>
        </h1>
        <h1 className="text-white text-lg font-semibold">
            <span className="text-2xl text-[#A582F7]">Salary:</span> ${job?.salary} per year
        </h1>
        </div>
